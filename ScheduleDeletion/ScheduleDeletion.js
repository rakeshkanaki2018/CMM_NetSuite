...
function cbForEachResult(searchResult) {	
	GLOBALS.numResults++;
	 try
	 {
		if(GLOBALS.numResults == 200) {
			
			var deploymentID = nlapiGetContext().getDeploymentId();
			var newDeploymentID = "customdeploy_schdeletetionso_2";
			if(deploymentID == "customdeploy_schdeletetionso_2") {
				newDeploymentID = "customdeploy_schdeletetionso_1";
			}
			var status = nlapiScheduleScript("customscript65", newDeploymentID);
			nlapiLogExecution("DEBUG", "Schedule Script Status",status);
		}
		if(GLOBALS.numResults > 490) {
			return false;
		}
...
//	var context = nlapiGetContext();
	...
	if ((remainingUsage*1.0) <= (GLOBALS.govThresh*1.0)) {
		GLOBALS.numResultsPerRequest = 0;
